...
#import "ViewControllerCompleteRaspadinha.h"

#define RandomNumber(min, max) min + arc4random_uniform(max - min + 1)

@interface ViewControllerCompleteRaspadinha ()
...
@implementation ViewControllerCompleteRaspadinha

@synthesize collectionViewMenu, imvReward, lblResult, viewScratch, scratchList, blockInteraction, goodPlay, forcePremium;;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    ScrathItem *item = [ScrathItem newItemWithRect:collectionViewMenu.frame imageItem:nil  imageTexture:[UIImage imageNamed:@"items-collection.png"] lineThickness:self.lineThickness premium:false delegate:self order:0];
    [viewScratch addSubview:item.scrathView];
    [viewScratch bringSubviewToFront:item.scrathView];
    ...
    if (!blockInteraction){
        if (maskingProgress > 0.9){
            ...
            if (goodPlay){
                ...
                icon = [ToolBox graphicHelper_ImageWithTintColor:[UIColor whiteColor] andImageTemplate:icon];
                [alert addButton:@"OK" withType:SCLAlertButtonType_Normal actionBlock:^{
                    [self.navigationController popViewControllerAnimated:YES];
                }];
                [alert showCustom:icon color:[UIColor colorWithRed:41.0/255.0 green:0.0/255.0 blue:102.0/255.0 alpha:1.0] title:@"" subTitle:@"Não foi desta vez, quem sabe no próximo pagamento." closeButtonTitle:nil duration:0.0];
...
//- (void)mdScratchImageViewTouchesBegan:(MDScratchImageView *)scratchImageView
//{
//    NSLog(@"mdScratchImageViewTouchesBegan");
//}
//
//- (void)mdScratchImageViewTouchesMoved:(MDScratchImageView *)scratchImageView
//{
//    NSLog(@"mdScratchImageViewTouchesMoved");
//}
//
//- (void)mdScratchImageViewTouchesEnded:(MDScratchImageView *)scratchImageView
//{
//    NSLog(@"mdScratchImageViewTouchesEnded");
//}
