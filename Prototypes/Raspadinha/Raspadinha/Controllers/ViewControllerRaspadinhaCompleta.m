...
#import "ViewControllerRaspadinhaCompleta.h"

#define RandomNumber(min, max) min + arc4random_uniform(max - min + 1)

@interface ViewControllerRaspadinhaCompleta ()
...
@property (nonatomic, strong) ScrathItem *mainItem;

@end

@implementation ViewControllerRaspadinhaCompleta

@synthesize collectionViewMenu, imvReward, lblResult, viewScratch, scratchList, blockInteraction, goodPlay, forcePremium, mainItem;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    mainItem = [ScrathItem newItemWithRect:collectionViewMenu.frame imageItem:nil  imageTexture:[UIImage imageNamed:@"items-collection.png"] lineThickness:self.lineThickness premium:false delegate:self order:0];
    [viewScratch addSubview:mainItem.scrathView];
    [viewScratch bringSubviewToFront:mainItem.scrathView];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //
    [item.scrathView setExternalUIResponder:false];
...
- (void)mdScratchImageView:(MDScratchImageView *)scratchImageView didChangeMaskingProgress:(CGFloat)maskingProgress
{
    ...
    if (!blockInteraction && scratchImageView != mainItem.scrathView){
        if (maskingProgress > 0.85){
            
            if (goodPlay){
                
                //O usuário está com uma raspadinha premiada
                
                ScrathItem *item = [scratchList objectAtIndex:scratchImageView.tag];
                if (item.isPremium){
