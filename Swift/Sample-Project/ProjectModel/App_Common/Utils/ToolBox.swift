...
final class ToolBoxValidation{
    ...
    /** Verifica se um dado texto possui conteúdo válido. */
    class func textCheckRelevantContent(text:String?) -> Bool {
        ...
        if let string = text {
            ...
            let inverted1:CharacterSet = CharacterSet.whitespacesAndNewlines.inverted
            if let _:Range = string.rangeOfCharacter(from: inverted1) {
                let inverted2:CharacterSet = CharacterSet.illegalCharacters.inverted
                if let _:Range = string.rangeOfCharacter(from: inverted2) {
                    return true
                } else {
                    return false
                }
